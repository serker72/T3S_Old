...
function tzs_front_end_my_shipments_handler($atts) {
	...
	if ($user_id == 0) {
		...
		$sql = "SELECT COUNT(*) as cnt FROM ".TZS_SHIPMENT_TABLE." WHERE user_id=$user_id AND active=1;";
		...
		if (count($res) == 0 && $wpdb->last_error != null) {
			...
			$from = ($page-1) * $pp;
			$sql = "SELECT * FROM ".TZS_SHIPMENT_TABLE."  WHERE user_id=$user_id AND active=1 ORDER BY time DESC LIMIT $from,$pp;";
			...
			if (count($res) == 0 && $wpdb->last_error != null) {
				...
				<tr>
					...
					<th id="descr">Описание</th>
					<th id="wight">Вес</th>
					<th id="vol">Объем</th>
					<th id="sum">Количество машин</th>
				...
				<button onClick="javascript: window.open('/account/add-shipment/', '_self');">Добавить груз</button>
